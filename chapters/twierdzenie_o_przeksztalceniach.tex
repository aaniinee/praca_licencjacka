...
\begin{defi} % 1.10.9
    ...
    Wtedy, powiemy że $f$ jest \emph{$\epsilon$-przekształceniem}, jeżeli dla każdego punktu w $Y$ średnica przeciwobrazu w tym punkcie jest mniejsza od $\varepsilon$, czyli  $\forall y\in Y \; \diam (f^{-1}(y))<\varepsilon$.
\end{defi}
\begin{przy}
...
\begin{przy}
    Dla $n\ge 3$, weźmy pokrycie odcinka $(0,1)=I$ $U=\{(\frac{i-1}{n}, \frac{i+1}{n})\:| \; i=1, \ldots, n-1\}$. \\
...
\end{twie}
\begin{proof}
    Weźmy dowolne otwarte pokrycie $\mathcal{U}$ przestrzeni $X$. Teraz niech $\varepsilon>0$ będzie liczbą Lebesgue'a pokrycia $\mathcal{U}$ - czyli taka wielkość, że dla każdego podzbioru $A\subset X$, jeżeli średnica $\delta(A)$ jest mniejsza od $\varepsilon$, to $A$ zawiera się w którymś elemencie pokrycia $\mathcal{U}$.\\
...
\begin{proof} 
    ...
    $(\Rightarrow)$\\
    Mamy zwartą przestrzeń metryczną $X$ o wymiarze pokryciowym $\dim X\le n$ oraz ustalmy liczbę $\varepsilon>0$. Weźmy pokrycie $\mathcal{U}$ będące skończonym wpisaniem pokrycia $\{ \mathcal{B}(x, \frac{\varepsilon}{3}) \}_{x\in X}$. Z twierdzenia \ref{twie:o_map} wiemy, że możemy znaleźć $\mathcal{U}$-mapę $f:X\rightarrow Y$  przestrzeni $X$ w wielościan wymiaru co najwyżej $n$. Pozostaje w takim razie pokazać, że jest to również $\varepsilon-$mapa. \\
...
\begin{przy}
    ...
    $X_i$ to będą indeksowane od 0 zbiory dyskretne $2^i$, a funkcje $\pi _i^{i+1}$ będą określone przez równania $\pi _i^{i+1}(x_{i+1, 2j})= x_{i, j}$, $\pi _i^{i+1}(x_{i+1, 2j+1})= x_{i, j}$. Zauważmy, że dla takiego systemu, możemy znaleźć intuicyjny homeomorfizm między jego granicą a zbiorem cantora w reprezentacji korzystającej z systemu trójkowego.
    \begin{figure}[H]
        \centering
