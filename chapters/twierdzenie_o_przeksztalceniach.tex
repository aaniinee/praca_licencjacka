...
\begin{defi} % 1.10.9
    ...
    Wtedy, powiemy że $f$ jest \emph{$\epsilon$-przekształceniem}, jeżeli dla każdego punktu w $Y$ średnica przeciwobrazu w tym punkcie jest miejsza od $\varepsilon$, czyli  $\forall y\in Y \; \diam (f^{-1}(y))<\varepsilon$ 
\end{defi}
\begin{przy}
...
\begin{defi} % 1.10.8
    ...
    Powiemy że $f$ jest \emph{$\mathcal{U}$-mapą}, jeżeli istnieje $\mathcal{V}$ takie  otwarte pokrycie $Y$, że przeciwobraz $f^{-1}(\mathcal{V})$ jest zwężeniem pokrycia $\mathcal{U}$.
\end{defi}
...
\begin{proof} 
    ...
    $(\Rightarrow)$\\
    Mamy zwartą przestrzeń metryczną $X$ o wymiarze pokryciowym $\dim X\le n$ oraz ustalmy liczbę $\varepsilon>0$. Weźmy pokrycie $\mathcal{U}$ będące skończonym wpisaniem pokrycia $\{ \mathcal{B}(x, \frac{\varepsilon}{2}) \}_{x\in X}$. Z twierdzenia \ref{twie:o_map} wiemy, że możemy znaleźć $\mathcal{U}$-mapę $f:X\rightarrow Y$  przestrzeni $X$ w wielościan wymiaru co najwyżej $n$. Pozostaje w takim razie pokazać, że jest to również $\varepsilon-$mapa. \\
...
\begin{przy}
    Pokażemy, że wymiar okręgu jednostkowego równy jest $1$, poprzez homeomorfizm z granicą ciągu wielokątów. $X_i$ to będą obwody $2^i$-kątów foremnych o wspólnych wierzchołkach znajdujących się na tym samym okręgu, a funkcie $\pi_j^j$ - rzut radialny. Wtedy oczywiście dla każdego $i$ spełnione jest $\dim X_i = 1$, więc okrąg, który jest homeomorficzny z granicą tego odwróconego ciągu, również ma wymiar 1.
    \begin{figure}[H]
        \centering
        \input{pictures/okrag.tex}
...
\begin{przy}
    ...
    $X_i$ to będą indeksowane od 0 zbiory dyskretne $2^i$, a funcje $\pi _i^{i+1}$ będą określone przez równania $\pi _i^{i+1}(x_{i+1, 2j})= x_{i, j}$, $\pi _i^{i+1}(x_{i+1, 2j+1})= x_{i, j}$. Zauważmy, że dla takiego systemu, możemy znaleźć intuicyjny homeomorfizm między jego granicą a zbiorem cantora w reprezentacji kożystającej z systemu trójkowego.
    \begin{figure}[H]
        \centering
        \input{pictures/cantor.tex}
