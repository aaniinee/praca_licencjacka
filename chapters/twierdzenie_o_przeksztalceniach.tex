...
\begin{defi}
    Dla odwróconego ciągu $S = \{X_i, \pi_j^i, N\}$, \emph{nicią} nazwiemy element $\{x_i\}_{i\in N}$, $x_i \in X_i$, jeżeli dla każdych indeksów $i, j\in N$, $j\le i$ spełnione jest $\pi _j^i(x_i)=x_j$. Teraz podzbiór przestrzeni $\prod_{i\in N}X_i$, składający się ze wszystkich nici $S$ nazwiemy \emph{granicą odwrotną systemu odwrotnego $S$} (ozn. $\varprojlim S$).
\end{defi}
