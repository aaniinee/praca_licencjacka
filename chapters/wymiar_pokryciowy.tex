...
Łatwo możemy zauważyć, że jeżeli przestrzenie $X$ i $Y$ są homeomorficzne, to oczywiście spełniają $\dim X = \dim Y$, kożystamy tutaj z własności zamknięcia homeomorfizmu na zbiory otwarte, jak również że dla wszystkich przestrzeni euklidesowych 
$\dim \mathbb{R}^n=n$, która została udowodniona w \cite{bro1913} (pokazane zostało $\dim I^n=n$).
\begin{przy}
    Zobaczmy, że dla każdego pokrycia przestrzeni euklidesowej $\mathbb{R}^1\cong \mathbb{R}$, znajdziemy jak na rusunku \ref{fig:wymiar_1}, które jest oczywiście krotności 1. 
    \begin{figure}[H]
        \centering
...
\end{proof}
Łatwo możemy zauważyć że podobna własność spełniona będzie dla wymiaru pokryciowego, czyli że dla każdej podrzestrzeni z topologią dziedziczoną $M$ przestrzeni $X$ spełnione będzie $\dim M\le \dim X$.
