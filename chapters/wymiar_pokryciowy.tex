...
\begin{defi}
    Pokrycie $\mathcal{B}$ jest \emph{wpisane} w pokrycie $\mathcal{A}$ tej samej przestrzeni $X$, jeżeli dla każdego $B\in \mathcal{B}$ istnieje $A\in \mathcal{A}$, takie że $B\subset A$.
    \begin{figure}[H]
        \centering
...
\begin{defi}
    ...
    \end{figure}
\end{defi}
Zauważmy, że każde zwężenie jest również wpisaniem.
...
\end{wnio}
Możemy łatwo zauważyć, że rodzina o krotności $-1$ musi zawierać jedynie zbiory puste, a rodzina o krotności 0 stworzona jest ze zbiorów parami rozłącznych, w których nie wszystkie są puste.
\\
...
\begin{defi}    
    ...
    \begin{itemize}
        \item[(ČL1)] $\dim X\leq n$ dla $n=-1,0,1,\cdots$, jeśli dla każdego skończonego otwartego pokrycia przestrzeni $X$ istnieje skończone otwarte wpisanie krotności $\leq n$,
        \item[(ČL2)] $\dim X=n$ jeżeli spełnione są nierówności $\dim X\leq n$ oraz $\dim X> n-1$,
        \item[(ČL3)] $\dim X=\infty$, jeżeli dla każdego $n=-1,0,1,\ldots$ zachodzi $\dim X >n$.
    \end{itemize} 
\end{defi}
\emph{ČL} pochodzi tutaj od nazwisk Čech i Lebesgue, którzy, jak wspomniane było wcześniej, jako pierwsi wprowadzili definicję wymiaru pokryciowego.\\
Łatwo możemy zauważyć, że jeżeli przestrzenie $X$ i $Y$ są homeomorficzne, to oczywiście spełniają $\dim X = \dim Y$, korzystamy tutaj z własności zamknięcia homeomorfizmu na zbiory otwarte, jak również że dla wszystkich przestrzeni euklidesowych 
$\dim \mathbb{R}^n=n$, która została udowodniona w \cite{bro1913} (pokazane zostało $\dim I^n=n$).
\begin{przy}
    Zobaczmy, że dla każdego pokrycia przestrzeni euklidesowej $\mathbb{R}^1\cong \mathbb{R}$, znajdziemy wpisanie skonstruowane w sposób pokazany na rysunku \ref{fig:wymiar_1}, które jest oczywiście krotności 1. 
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{pictures/wymiar_1.png}
        \caption{Pokrycie odcinka krotności 1}
        \label{fig:wymiar_1}
    \end{figure}
\end{przy}
\begin{przy}
    Zauważmy, że dla każdego pokrycia $\mathcal{U}$ kwadratu będącego podzbiorem $\mathbb{R}^2$, możemy znaleźć pokrycie skonstruowane jak na rysunku \ref{fig:kwadrat}, tak że dla odpowiednio małych elementów będzie ono wpisane w pokrycie $\mathcal{U}$. Oczywiście to pokrycie jest krotności 2.

     \begin{figure}[H]
        \centering
        \input{pictures/wpisanie_kwadratu.tex}
        % \includesvg[width=0.2\textwidth]{pictures/Refinement_on_a_planar_shape.svg}
        \caption{Pokrycie kwadratu krotności 2}
        \label{fig:kwadrat}
    \end{figure}
\end{przy}
\section{Twierdzenia}
Pokażemy teraz dwa twierdzenia, pierwsze mówi o tym, że zamiast szukać wpisań możemy szukać zwężeń, drugie pokazuje, że nie musimy pokazywać istnienia zwężeń dla pokryć o dowolnie dużej liczbie elementów, tylko rozważać jedynie te o ograniczonej.
\begin{twie} % 1.6.8
    Dla każdej przestrzeni normalnej $X$ następujące warunki są równoważne:
    \begin{itemize}
...
\begin{proof}
    ...
    Zaczniemy od zdefiniowania przyporządkowania $i(V)$ - taki indeks z przedziału $\{1,\ldots k\}$, dla którego $V\subset U_{i(V)}$. Teraz dla każdego $i\in \{1, \ldots, k\}$ zdefiniujmy $V_i = \bigcup\{V\;|\;i=i(V)\}$. Pokażemy, że tak zdefiniowana rodzina $\tilde{\mathcal{V}}=\{V_i\}_{i=1}^k$ jest szukanym zwężeniem. \\
...
\begin{proof}
    ...
    Do tego możemy założyć, że jeżeli $\mathcal{V}' = \{V_i'\}_{i=1}^k$ to otwarte zwężenie pokrycia $\mathcal{V}$, to spełnia ono: 
...
\begin{wnio}
    Do pokazania, że przestrzeń $X$ ma wymiar pokryciowy mniejszy równy $n$, wystarczy rozważać otwarte pokrycia $n+2$ elementowe.
\end{wnio}
% \begin{proof}
%     Jeżeli $\dim X\le n$, 
% \end{proof}
% todo nie jestem pewna co?
...
\begin{defi}
    ...
    \begin{itemize}
        ...
        \item[(MU2)] $\ind X\le n$ dla $n=0,1,\ldots$ jeżeli dla każdego punktu $x\in X$ i każdego jego otoczenia $V$ istnieje pewien zbiór otwarty $U\subset X$ spełniający
        \[x\in U\subset V\;\;\;\mathrm{oraz}\;\;\; \ind \Fr U \le n-1,\]
        \item[(MU3)] $\ind X=n$ jeżeli zachodzą nierówności $\ind X\leq n$ oraz $\ind X> n-1$,
...
\end{proof}
Łatwo możemy zauważyć że podobna własność spełniona będzie dla wymiaru pokryciowego, czyli że dla każdej podprzestrzeni z topologią dziedziczoną $M$ przestrzeni $X$ spełnione będzie $\dim M\le \dim X$.
...
\begin{defi}
    ...
    \begin{itemize}
        ...
        \item[(BČ2)] $\Ind X\le n$ dla $n=0,1,\ldots$ jeżeli dla każdego zbioru domkniętego $A$ w $X$ i każdego jego otwartego otoczenia $V$ istnieje pewien zbiór otwarty $U\subset X$ spełniający
        \[A\subset  U\subset V\;\;\;\mathrm{oraz}\;\;\; \Ind \Fr U \le n-1,\]
        \item[(BČ3)] $\Ind X=n$ jeżeli zachodzą nierówności $n-1<\Ind X $ oraz $\Ind X \le n$,
        \item[(BČ4)] $\Ind X=\infty$, jeżeli dla każdego $n=-1,0,1,\ldots$ zachodzi $\Ind X >n$.
    \end{itemize}
\end{defi}
Skrót \emph{BČ} pochodzi tutaj od nazwisk Brouwer i Čech \cite{bro1913}, \cite{cech1993}. 
Łatwo możemy pokazać, że podobnie jak dla poprzedniej definicji, dla homeomorficznych przestrzeni normalnych $X\cong Y$ zachodzi $\Ind X=\Ind Y$. Oczywiście też dla każdej przestrzeni normalnej $X$ spełniona jest również nierówność $\ind X\le \Ind X$, 
jako iż warunek (BČ2) w definicji $\Ind$ implikuje warunek (MU2) w $\ind$.

Ważne twierdzenie, wiążące te trzy wymiary ze sobą to:
\begin{twie}
    Dla każdej przestrzeni metrycznej ośrodkowej $X$ spełnione są równości
    \[\dim X = \ind X = \Ind X.\]
\end{twie}
\begin{proof}
    dowód możemy znaleźć w \cite{hurewicz1941}.
\end{proof}
Mimo że dla przestrzeni metrycznych ośrodkowych te wymiary pokrywają się, w ogólności, na dużo bardziej skomplikowanych przestrzeniach, przestają mieć taką samą wartość.
Wiemy jednak, że dla przestrzeni \emph{T1} spełnione są $\ind X \le \Ind X$, a dla przestrzeni normalnych $\dim X\le \Ind X$.