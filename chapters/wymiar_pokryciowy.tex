...
\begin{defi}
    Pokrycie $\mathcal{B}$ jest \emph{wpisane} w pokrycia $\mathcal{A}$ tej samej przestrzeni $X$, jeżeli dla każdego $B\in \mathcal{B}$ istnieje $A\in \mathcal{A}$, takie że $B\subset A$.
    \begin{figure}[H]
        \centering
...
\begin{przy}
    Zobaczmy, że dla każdego pokrycia przestrzeni euklidesowej $\mathbb{R}^1\cong \mathbb{R}$, znajdziemy jak na rysunku \ref{fig:wymiar_1}, które jest oczywiście krotności 1. 
    \begin{figure}[H]
        \centering
...
Łatwo możemy pokazać, że podobnie jak dla poprzedniej definicji, dla homeomorficznych przestrzeni normalnych $X\cong Y$ zachodzi $\Ind X=\Ind Y$. Oczywiście też dla każdej przestrzeni normalnej $X$ spełniona jest nierówność $\ind X\le \Ind X$, 
...
\end{proof}
Mimo że na przestrzeniach metrycznych ośrodkowych te wymiary pokrywają się, w ogólności, na dużo bardziej skomplikowanych przestrzeniach przestają mieć taką samą wartość.
