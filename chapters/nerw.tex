...
\begin{proof}
    ...
    Zacznijmy od zdefiniowania punktu $p_1:=q_1$. Każdy kolejny punkt będziemy konstuować korzystać z poprzedno ustalonych. Załóżmy więc, że punkty $p_1, \ldots, p_{j-1}$ są dobrze zdefiniowane, czyli tworzą układ w położeniu ogólnym oraz spełniają $\rho(p_i, q_i)<\alpha$ dla $i=1, \ldots, j-1$.\\
    ...
    \\
    Zauważmy teraz, że dowód dla $l>0$ przebiega analogicznie, każdy punkt $p_j$, $j=1, \ldots, k$ dobieramy tak by tworzył z punktami $r_1, \ldots, r_l, p_1, \ldots p_{j-1}$ układ w położnieniu ogólnym. 
    \begin{figure}[H]
        \centering
...
\begin{proof}
    ...
    \}
    \]
    Teraz pozostaje pokazć, że tak zdefiniowana rodzina jest nerwem $\mathcal{N(U)}$ oraz że $\mathcal{N(U)}\cap H=\emptyset$.\\
