...
\begin{defi}
    Powiemy, że punkty $p_1, p_2, \ldots , p_k \in \mathbb{R}^m$ są w \emph{położeniu ogólnym}, jeżeli dla każdego ciągu $l+1$ liczb naturalnych $0< i_0<i_1<\cdots<i_l\le k$, $l\le m$, punkty $p_{i_0}, p_{i_1},\ldots, p_{i_l}$ tworzą układ afinicznie niezależny.
\end{defi}
\begin{przy}
    ...
    \begin{figure}[H]
        ...
        \input{pictures/polozenie_ogolne.tex}
        \caption{Układ w położeniu ogólnym.}
        \label{fig:polozenie_ogolne}
    \end{figure}
    Na rysunku \ref{fig:polozenie_ogolne} punkty $p_1, \ldots, p_7$ tworzą oczywiście taki układ.
\end{przy}
...
\end{twie}
\begin{proof}
    Zaczniemy od pokazania dowodu dla $l=0$, czyli bez brania pod uwagę punktów $r_j$. \\
    Zacznijmy od zdefiniowania punktu $p_1:=q_1$. Każdy kolejny punkt będziemy konstruować korzystać z poprzednio ustalonych. Załóżmy więc, że punkty $p_1, \ldots, p_{j-1}$ są dobrze zdefiniowane, czyli tworzą układ w położeniu ogólnym oraz spełniają $\rho(p_i, q_i)<\alpha$ dla $i=1, \ldots, j-1$.\\
    ...
    \\
    Zauważmy teraz, że dowód dla $l>0$ przebiega analogicznie, każdy punkt $p_j$, $j=1, \ldots, k$ dobieramy tak by tworzył z punktami $r_1, \ldots, r_l, p_1, \ldots p_{j-1}$ układ w położeniu ogólnym. 
    \begin{figure}[H]
        \centering
        \input{pictures/konstrukcja.tex}
...
\begin{defi}
    ...
    \[p_{i_0}p_{i_1} \ldots p_{i_m}\in \mathcal{N(U)} \Longleftrightarrow U_{i_0}\cap U_{i_1}\cap \cdots \cap U_{i_m}\neq \emptyset. \]
    \begin{figure}[H]
        \centering
        % \includegraphics[width=0.8\textwidth]{pictures/nerw_2.png}
        \input{pictures/nerw.tex}
        \caption{Nerw pokrycia}
        \label{fig:nerw_2}
    \end{figure}
\end{defi}

\section{Twierdzenie o zanurzeniu w $R^{2n+1}$}

\begin{twie}
    Niech $\mathcal{U}=\{U_i\}_{i=1}^k$ będzie skończonym otwartym pokryciem przestrzeni topologicznej $X$. Jeżeli krotność pokrycia $\mathcal{U}$ jest co najwyżej $n$, to nerw pokrycia $\mathcal{N(U)}$ możemy zrealizować w $\mathbb{R}^{2n+1}$. \\
    Ponadto, mając:
    \begin{itemize}
...
\begin{proof}
    ...
    Teraz niech $\mathcal{K}$ będzie rodziną sympleksów 
    zdefiniowanych w ten sposób, czyli 
    \[
    \mathcal{K} = 
    \{
        \Delta (p_{i_0},\ldots p_{i_l})\;\; | \;\; U_{i_0}\cap\cdots \cap U_{i_l}\ne \emptyset
    \}
    \]
    Teraz pozostaje pokazać, że tak zdefiniowana rodzina jest nerwem $\mathcal{N(U)}$ oraz że $\mathcal{N(U)}\cap H=\emptyset$.\\
    ...
    oraz by punkty
    \[p_{i_a}, \ldots, p_{i_l}, p_{j_a},\ldots,  p_{j_m}\]
    były parami różne.\\
    ...
    \[p=\sum _{t=0}^m \lambda'_tp_{j_t}, \;\; \sum_{t=0}^m\lambda'_t=1,
    \lambda'_t\ge 0\forall_{t=0,1, \ldots, m} \]
    Dostajemy w takim razie układ równań:
    \[\sum _{s=0}^l \lambda_sp_{i_s}- \sum _{t=0}^m \lambda'_tp_{j_t} = 
    0\]
    \[\sum _{s=0}^l \lambda_s - \sum _{t=0}^m \lambda'_t = 0\]
    Pierwsze równanie możemy uprościć do postaci:
    \[
    \sum _{s=0}^{a-1} (\lambda_s-\lambda'_s)p_{i_s} +
    \sum _{s=a}^l \lambda_sp_{i_s}- \sum _{t=a}^m \lambda'_tp_{j_t} = 
    0\]
    Zauważmy, że skoro $l, m\le n$, czyli $2+m+l\le 2n+2$, to punkty 
    $p_{i_0}, \ldots, p_{i_l},p_{j_a}\ldots ,p_{j_m}$ będą tworzyły układ afinicznie niezależny, z czego wynika że wszystkie współczynniki w powyższym równaniu muszą być równe 0, za czym idzie:
    ...
    czyli dla $a>0$ punkt $p$ należy również do sympleksu opartego na $p_{i_0}, \ldots, p_{i_{a-1}}$, który jest ścianą obu sympleksów $S_1, S_2$. Zauważmy, że jeżeli $a=0$, to oczywiście układ $p_{i_0}, \ldots, p_{i_l},p_{j_0}\ldots ,p_{j_m}$ będzie afinicznie niezależny, za czym idzie że wszystkie współczynniki muszą być zerowe, co jest sprzeczne z tym że ich suma jest równa $1$. Więc sympleksy $S_1$ i $S_2$ muszą mieć puste przecięcie, co kończy dowód.\\
